...
use crate::{interpreter::{LoxCallable}, lox::{Lox, LoxError, LoxErrorType}, parser::{Expression, LoxObject}};
...
fn clock_impl(lox: &mut Lox, _args: Vec<LoxObject>) -> Result<LoxObject, LoxError> {
    match SystemTime::now().duration_since(UNIX_EPOCH) {
        ...
        Err(_err) => Err(lox.runtime_error(Expression::Literal(LoxObject::Nil), LoxErrorType::SystemError, "error getting system time"))
    }
}
